...
package body AWS.HPACK.Huffman is
...
--            Integer_Text_IO.Put (Integer (Bits (K)), Width => 1);
            declare
...
--         New_Line;
   ...
   function Encode (Str : String) return Stream_Element_Array is
      ...
      begin
--         Integer_Text_IO.Put (Positive (V), Base => 16);
         I := I + 1;
         R (I) := V;
         V := 0;
         B := 8;
...
--            Put_Line (B'Img & N'Img);
   ...
   exception
      when E: others =>
         Put_Line ("crash!" & Exception_Information (E));
         return R;
   end Encode;

begin
   Create_Tree;
end AWS.HPACK.Huffman;

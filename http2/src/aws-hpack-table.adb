...
package body AWS.HPACK.Table is

   use Ada;
   use Ada.Containers;

   SETTINGS_HEADER_TABLE_SIZE : constant := 4_096;
   ...
   function D_Key (Index : Positive) return String is  ("@" & Index'Image);
   ...
   begin
      ...
      Text_IO.Put_Line ("@@@ Dynamic Table : "
                        & D.Length'Img & " (s =" & D.Size'Img & ')');
   end Dump;
   ...
   begin
      ...
      elsif Index - 61 <= Self.Dynamic.Length then
   ...
   function Get_Name_Index (Self : Object; Name : String) return Positive is
   begin
      if Self.Static.T_NI.Contains (Name) then
         return Self.Static.T_NI (Name).Index;

      elsif Self.Dynamic.T_NI.Contains (Name) then
         declare
            Id : constant Ids := Self.Dynamic.T_NI (Name);
         begin
   ...
   procedure Insert_Static (Self : in out Object; Name, Value : String) is
      Item : Name_Value := (Name'Length, Value'Length, Name, Value);
   begin
...
--         Index_Img : constant String := D_Key (Index);
      begin
         Self.Dynamic.T_NI.Include (Name, Id);
